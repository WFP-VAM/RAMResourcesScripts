...
gen FCSCat28 = .
replace FCSCat28 = 1 if FCS <= 28
replace FCSCat28 = 2 if FCS > 28 & FCS <= 42
replace FCSCat28 = 3 if FCS > 42

*OR

recode FCS (min/28 = 1)(28.5/42 = 2)(42.5/max = 3), generate(FCSCat28)

label variable FCSCat28 "FCS Categories: High thresholds - for IPC"

*** Define value labels and properties for "FCS Categories"

label define FCSCat28_lbl 1 "Poor" 2 "Borderline" 3 "Acceptable"
label values FCSCat28 FCSCat28_lbl

*** Check distribution of final categories

tabulate FCSCat28, missing
...
gen FCSCat28IPC = .
replace FCSCat28IPC = 1 if FCS <= 28
replace FCSCat28IPC = 2 if FCS > 28  & FCS <= 42
replace FCSCat28IPC = 3 if FCS > 42

label variable FCSCat28IPC "Official IPC Classification for FCS - high thresholds"

label define FCSCat28IPC_lbl 1 "Poor - IPC Phase 4-5" 2 "Borderline - IPC Phase 3" 3 "Acceptable - IPC Phase 1-2"

label values FCSCat28IPC FCSCat28IPC_lbl

*** Check distribution of final categories

tabulate FCSCat28IPC, missing
