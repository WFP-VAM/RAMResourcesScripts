...
***This is only a syntax to calculate an example of an LCS-FS indicator. IMPORTANT to ensure that the design of the LCS-FS module considers the coping capacities present in the local context/among target population and doesnt necessarily follow this example selection of strategies (unless relevant). 
...
do if Lcs_stress_DomAsset = 20 | Lcs_stress_DomAsset = 30 | Lcs_stress_Saving  = 20 | Lcs_stress_Saving  = 30 | Lcs_stress_EatOut =20 | Lcs_stress_EatOut =30 |  Lcs_stress_CrdtFood  =20 | Lcs_stress_CrdtFood=30.
