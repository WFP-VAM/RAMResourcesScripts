...
#can only download repeat csv data as zip file from moda with group names
#will update this code with more elegant solution to remove group names or if you download as SPSS you can skip this step
...
data <- data %>%
  mutate(across(c(PWMDDWStapCer,PWMDDWStapRoo,PWMDDWPulse,PWMDDWNuts,PWMDDWMilk,
                  PWMDDWDairy,PWMDDWPrMeatO,PWMDDWPrMeatF,PWMDDWPrMeatPro,
                  PWMDDWPrMeatWhite,PWMDDWPrFish,PWMDDWPrEgg,PWMDDWVegGre,
                  PWMDDWVegOrg,PWMDDWFruitOrg,PWMDDWVegOth,PWMDDWFruitOth,
                  PWMDDWSnf,PWMDDWFortFoodoil,PWMDDWFortFoodwflour,
...
# Calculate 2 MDDW indicators -------------------------------------------------#
#Calculate 2 MDDW indicators based on WFP guidelines https://docs.wfp.org/api/documents/WFP-0000140197/download/ pg.8
#Standard MDDW Indicator for population based surveys 
#counts SNF in home group (refer to https://docs.wfp.org/api/documents/WFP-0000139484/download/ for "home group")
...
data <- data %>% mutate(
  MDDW_Staples = case_when(
    PWMDDWStapCer == 1 |  PWMDDWStapRoo == 1 | PWMDDWSnf == 1 | 
    PWMDDWFortFoodwflour == 1 | PWMDDWFortFoodmflour == 1 | 
    PWMDDWFortFoodrice == 1 | PWMDDWFortFooddrink == 1 ~ 1, TRUE ~ 0),
  ...
  MDDW_MeatFish  = case_when(
    PWMDDWPrMeatO == 1 | PWMDDWPrMeatF == 1 | PWMDDWPrMeatPro == 1 | 
    PWMDDWPrMeatWhite == 1 | PWMDDWPrFish == 1 ~ 1, TRUE ~ 0),
  MDDW_Eggs  = case_when(
...
data <- data %>% mutate(
  ...
  MDDW_MeatFish_wfp  = case_when(
    PWMDDWPrMeatO == 1 | PWMDDWPrMeatF == 1 | PWMDDWPrMeatPro == 1 | 
    PWMDDWPrMeatWhite == 1 |  PWMDDWPrFish == 1 | PWMDDWSnf == 1 ~ 1, TRUE ~ 0),
...
data <- data %>% mutate(MDDW = MDDW_Staples + MDDW_Pulses + MDDW_NutsSeeds + 
                               MDDW_Dairy + MDDW_MeatFish + MDDW_Eggs + 
                               MDDW_LeafGreenVeg + MDDW_VitA + MDDW_OtherVeg +
                               MDDW_OtherFruits)
...
#WFP MDDW method for program monitoring - SNF will count in the meats group
data <- data %>% mutate(MDDW_wfp = MDDW_Staples_wfp + MDDW_Pulses_wfp + 
                                   MDDW_NutsSeeds_wfp + MDDW_Dairy_wfp +
                                   MDDW_MeatFish_wfp + MDDW_Eggs_wfp + 
                                   MDDW_LeafGreenVeg_wfp + MDDW_VitA_wfp + 
