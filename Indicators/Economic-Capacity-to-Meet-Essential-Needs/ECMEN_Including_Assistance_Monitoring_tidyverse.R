...
# Version including assistance (for monitoring)
...
#   add link to standalone guidance when available
...
# Note: Remember that for the version of ECMEN used for monitoring (including 
# assistance), the value of consumed in-kind assistance and gifts should be 
# included from the household economic capacity aggregate.

if(EXP_total){
  
  # Aggregate food expenditures, value of consumed food from gifts/assistance, 
  # and value of consumed food from own production
  ...
  data$HHExpNF_ECMEN <- rowSums(data[c("HHExpNFTotal_Purch_MN_1M", 
                                      "HHExpNFTotal_GiftAid_MN_1M")],
                               na.rm = TRUE)
  data$HHExpNF_ECMEN <- ifelse(is.na(data$HHExpNF_ECMEN), 0, data$HHExpNF_ECMEN)
...
if(ECMEN_MEB){
  ...
  data$ECMEN_inclAsst <- ifelse((!is.na(data$PCExp_ECMEN) & !is.na(data$MEB)), 
                                (data$PCExp_ECMEN > data$MEB), NA)
  # Make sure to rename MEB variable as appropriate
  var_label(data$ECMEN_inclAsst)            <- "Economic capacity to meet essential needs - including assistance"
  
  val_lab(data$ECMEN_inclAsst) = num_lab("
             1 Above MEB
             0 Below MEB
          ")
  # Compute the indicator (use weights when applicable!)
  table(data$ECMEN_inclAsst)
...
if(ECMEN_SMEB){
  ...
  data$ECMEN_inclAsst_SMEB <- ifelse((!is.na(data$PCExp_ECMEN) & !is.na(data$SMEB)), 
                                     (data$PCExp_ECMEN > data$SMEB), NA)
  # Make sure to rename SMEB variable as appropriate
  var_label(data$ECMEN_inclAsst_SMEB)            <- "Economic capacity to meet essential needs - SMEB - including assistance"
  
  val_lab(data$ECMEN_inclAsst_SMEB) = num_lab("
             1 Above SMEB
             0 Below SMEB
          ")
  # Compute the indicator (use weights when applicable!)
  table(data$ECMEN_inclAsst_SMEB)
