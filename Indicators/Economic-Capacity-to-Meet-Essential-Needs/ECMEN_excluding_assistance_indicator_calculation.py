...
#                    (Excluding Assistance Version - Assessments)
#------------------------------------------------------------------------------#
...
# Note: Remember that for the version of ECMEN used for assessments (excluding assistance),
# the value of consumed in-kind assistance and gifts should be excluded from the household economic capacity aggregate.
# the calculation below is just for analytical purposes and will not be included in the calculation of ECMEN (Excluding assistance version)
...
df['HHExpF_ECMEN'] = df[['HHExp_Food_Purch_MN_1M', 'HHExp_Food_Own_MN_1M']].sum(axis=1)

# For Non-FOOD only expenditures are considered
df['HHExpNF_ECMEN'] = df['HHExpNFTotal_Purch_MN_1M']

# Aggregate food and non-food
df['HHExp_ECMEN'] = df[['HHExpF_ECMEN', 'HHExpNF_ECMEN']].sum(axis=1)

# After deudcting the value of in-kind assistance from gift/aid, we should deduct cash assistance
# Sum the amount of cash assistance received by WFP and other humanitarian partners
df['HHAsstCBTRec'] = df[['HHAsstWFPCBTRecTot', 'HHAsstUNNGOCBTRecTot']].sum(axis=1)

# Express in monthly terms if the assistance is reported in the last 3 monts recall period
df['HHAsstCBTRec_1M'] = df['HHAsstCBTRec'] / 3

# Estimate the median share of assistance used for consumption
HHAsstCBTCShare_med = df['HHAsstCBTCShare'].median()

# Multiply the cash assistance received by the median share used for consumption
df['HHAsstCBTRec_Cons_1M'] = df['HHAsstCBTRec_1M'] * (HHAsstCBTCShare_med / 100)

# Deduct the cash assistance from the household economic capacity
df['HHExp_ECMEN'] = df['HHExp_ECMEN'] - df['HHAsstCBTRec_Cons_1M'].fillna(0)
...
df['ECMEN_exclAsst'] = df['PCExp_ECMEN'] > df['MEB']
...
df['ECMEN_exclAsst_SMEB'] = df['PCExp_ECMEN'] > df['SMEB']

# _____________________________ LABEL VARIABLES (IF NEEDED) ______________________________________
df['HHAsstWFPCBTRecTot'].rename("Amount of cash assistance received from WFP - last 3 months", inplace=True)
df['HHAsstUNNGOCBTRecTot'].rename("Amount of cash assistance received by other humanitarian partners - last 3 months", inplace=True)
df['HHAsstCBTRec_Cons_1M'].rename("Amount of cash assistance received from WFP and other humanitarian partners - last 3 months", inplace=True)
df['HHAsstCBTCShare'].rename("Share of cash assistance spent on consumption", inplace=True)
df['ECMEN_exclAsst'] = df['ECMEN_exclAsst'].map({True: 'Above MEB', False: 'Below MEB'})
df['ECMEN_exclAsst_SMEB'] = df['ECMEN_exclAsst_SMEB'].map({True: 'Above SMEB', False: 'Below SMEB'})
...
df['ECMEN_exclAsst'].rename("Economic capacity to meet essential needs - excluding assistance", inplace=True)
